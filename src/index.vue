<template>
    <div class="lion" ref="page">
        <mouse ref="mouse"></mouse>
        <div class="text" ref="text">
            <P v-for="(v, i) in text" :key="i">{{v}}</P>
        </div>
        <div class="lions" ref="lion">
            <div class="lion-head">
                <div class="lion-eye left"></div>
                <div class="lion-eye right"></div>
                <div class="lion-mouse"></div>
                <span v-for="(v, i) in head" :key="i"></span>
            </div>
            <div class="lion-body">
                <span v-for="(v, i) in body" :key="i"></span>
            </div>
            <div class="lion-foot">
                <span v-for="(v, i) in foot" :key="i"></span>
            </div>
            <div class="lion-tail">
                <span v-for="(v, i) in tail" :key="i"></span>
            </div>
        </div>
        <router-link class="door" to="passage">enter</router-link>
    </div>
</template>

<script>
import Util from './util';
import Mouse from '@/components/bg-page';
...
export default {
    ...
    mounted () {
        ...
        // 确定中心
        this.head = Util.createSpot(232);
        this.body = Util.createSpot(300);
        this.foot = Util.createSpot(108);
        this.tail = Util.createSpot(12);
        this.$nextTick(() => {
            this.drawLion();
        });
    },
    methods: {
        ...
        roundPosition (rad, radius, item, centerX, centerY) {
        ...
        linePosition (centerX, centerY, item, r) {
            let x = centerX + r;
            item.style.left = x + 'px';
